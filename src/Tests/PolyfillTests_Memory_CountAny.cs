...
partial class PolyfillTests
{
    ...
    public Task ReadOnlySpan_CountAny()
    {
        ...
        if (span.CountAny(values) != 2)
            throw new Exception($"Expected CountAny(values) to be 2 but got {span.CountAny(values)}");
        if (span.CountAny('b', 'c') != 2)
            throw new Exception($"Expected CountAny('b', 'c') to be 2 but got {span.CountAny('b', 'c')}");
        if (span.CountAny(['b', 'c'], EqualityComparer<char>.Default) != 2)
            throw new Exception($"Expected CountAny with comparer to be 2");
