#if FeatureMemory && !NETCOREAPP3_0_OR_GREATER
...
static partial class Polyfill
{
    ...
    /// </summary>
    //Link: https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.rsa.importencryptedpkcs8privatekey#system-security-cryptography-rsa-importencryptedpkcs8privatekey(system-readonlyspan((system-byte))-system-readonlyspan((system-byte))-system-int32@)
    ...
    /// </summary>
    //Link: https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.rsa.importencryptedpkcs8privatekey#system-security-cryptography-rsa-importencryptedpkcs8privatekey(system-readonlyspan((system-char))-system-readonlyspan((system-byte))-system-int32@)
    public static void ImportEncryptedPkcs8PrivateKey(
        this RSA target,
    ...
    static class Pkcs8Helper
    {
        ...
        static byte[] DecryptPbes2(ReadOnlySpan<byte> password, byte[] data, ref int offset, int algIdEnd, int structureEnd)
        {
            ...
            {
                throw new CryptographicException("PBKDF2 with non-SHA1 hash requires .NET Core 2.0 or later");
            }

            using (var pbkdf2 = new Rfc2898DeriveBytes(password.ToArray(), salt, iterations))
            {
                derivedKey = pbkdf2.GetBytes(keyLength ?? keySize);
            }
#endif
