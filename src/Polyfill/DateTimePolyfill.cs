...
static partial class DateTimePolyfill
{
    ...
    public static bool TryParseExact(ReadOnlySpan<char> target, string format, IFormatProvider? provider, DateTimeStyles styles, out DateTime result) =>
#if NETSTANDARD2_1 || NETSTANDARD2_1_OR_GREATER || NET5_0_OR_GREATER
        DateTime.TryParseExact(target, format, provider, styles, out result);
#else
        DateTime.TryParseExact(target.ToString(), format, provider, styles, out result);
#endif
